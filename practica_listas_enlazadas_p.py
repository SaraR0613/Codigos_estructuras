...
def contar_vehiculos(via):
    motos = carros = buses = camiones = 0
    for v in via:
        if v.tipo == "moto":
            motos += 1
        elif v.tipo == "carro":
            ...
            camiones += 1
    print(f"\nMotos: {motos} | Carros: {carros} | Buses: {buses} | Camiones: {camiones}")

def buscar_por_placa(via, placa):
    for v in via:
        if v.placa == placa:
...
def eliminar_por_placa(via, placa):
    actual = via.head
    while actual:
        if actual.value.placa == placa:
            via.remove_node(actual)
            print(f"Vehículo con placa {placa} eliminado.")
            return
        actual = actual.next
    print(f"No se encontró vehículo con placa {placa}")

def ingresar_vehiculo(via, vehiculo):
    via.append(vehiculo)
    print(f"Nuevo vehículo ingresó: {vehiculo}")

def salir_vehiculo(via):
    if via.head:
        print(f"Vehículo salió: {via.head.value}")
        via.remove_node(via.head)

def avanzar_un_puesto(via):
    if via.head and via.tail:
        primero = via.head
        via.head = primero.next
        via.head.prev = None
        via.tail.next = primero
        primero.prev = via.tail
        primero.next = None
        via.tail = primero
        print("\nLos vehículos avanzaron un puesto.")


historial = []

def registrar(paso, detalle):
    historial.append(f"{paso}: {detalle}")

# En cada parte del código:
registrar("Paso preferencial", "Las motos prioridad 1 fueron adelantadas.")
...
def tiempo_total(via, distancia):
    total = 0
    for v in via:
        total += distancia / v.velocidad
    print(f"Tiempo estimado total: {total:.2f} segundos")
